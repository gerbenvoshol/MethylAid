...
##' targets <- read.metharray.sheet(baseDir)
##' data <- summarize(targets)
summarize_v2 <- function(targets, batchSize=-1, BPPARAM=NULL, rp.zero=TRUE,
                      verbose=TRUE, file=NULL, ...) {
    if(verbose)
        message(paste("Start summarization ..."))
    ...
    if(is.null(BPPARAM))
        {
            if(batchSize == -1)
                sData <- summarizeWholeBunch_v2(targets, rp.zero, verbose, ...)
            else
                sData <- summarizePerBatch_v2(targets, batchSize, rp.zero, verbose, ...)
        }
    else
        sData <- summarizeParallel_v2(targets, batchSize, BPPARAM, rp.zero, verbose, ...)
...
summarizeWholeBunch_v2 <- function(targets, rp.zero, verbose, ...)  {
    if(verbose)
        message("Summarize data in one go...")
...
summarizePerBatch_v2 <- function(targets, batchSize, rp.zero, verbose, ...){
    if(verbose)
        message("Summarize data in batches...")
...
summarizeParallel_v2 <- function(targets, batchSize,  BPPARAM, rp.zero, verbose, ...) {
    if(verbose)
        message("Summarize data in parallel...")
    ...
    if(batchSize == -1)
        sumParallel <- function(x)
            summarizeWholeBunch_v2(x, rp.zero=rp.zero, verbose=verbose, ...)
    else
        sumParallel <- function(x)
            summarizePerBatch_v2(x, batchSize=batchSize, rp.zero=rp.zero, verbose=verbose, ...)
